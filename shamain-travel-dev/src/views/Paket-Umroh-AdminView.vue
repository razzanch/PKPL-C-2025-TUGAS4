<template>
  <div class="paket-umroh-admin-view">
    ...
    <div
      class="main-content"
      :style="{ marginLeft: isSidebarCollapsed ? '70px' : '200px' }"
    >
      <!-- MAIN CONTENT START -->
        ...
        <form @submit.prevent="addProgram" class="barcode-form">
          ...
          <div class="system-counter-container">
            <div class="form-actions-up">
              <button type="submit" id="add-program" class="btn-add">
                Add
              </button>
              ...
              <template v-if="filteredPrograms.length > 0">
                ...
                >
                  ...
                  <td>
                    ...
                    <button
                      @click="openDeleteModal(program.id_program)"
                      class="delete-btn"
                    >
      ...
      <div v-if="isDeleteModalOpen" class="modal">
        <div class="modal-content">
          ...
          <input
            type="text"
            v-model="deleteForm.name_program"
            placeholder="Masukkan nama program"
          />
          ...
          <table>
            ...
            <tbody>
              <template v-if="filteredPrograms2.length > 0">
                <tr
                  v-for="packages in filteredPrograms2"
                  :key="packages.id_package"
                >
                  ...
                  <td>
                    <button
                      @click="openDeleteModal2(packages.id_package)"
                      class="delete-btn"
                    >
                      <img src="../assets/trash.png" alt="Delete" />
                    </button>
      ...
      <div v-if="isDeleteModalOpen2" class="modal">
        <div class="modal-content">
          ...
          <input
            type="text"
            v-model="deleteForm2.name_package"
...
export default {
  ...
  methods: {
    ...
    async fetchPrograms() {
      try {
        ...
        this.programs = response.data.data.filter(
          (program) =>
    ...
    async updateProgram() {
      try {
        const response = await api.put(
          `/programs/put/${this.editForm.id_program}`,
          {
        ...
        if (error.response && error.response.status === 409) {
          this.showNotification(
            "Program dengan nama ini sudah ada! Mohon gunakan nama lain",
            false
          );
        } else {
          this.showNotification(
            "Terjadi kesalahan saat memperbarui program!!",
            false
          );
        }
      }
    ...
    async deleteProgramConfirm() {
      ...
      try {
        const response = await api.delete(
          `/programs/delete/${this.deleteForm.id_program}`,
          {
        ...
        // Berhasil dihapus
        this.showNotification(
          response.data.message || "Program Berhasil Dihapus!",
          true
        );
        ...
        if (error.response) {
          ...
          switch (error.response.status) {
            ...
            case 409:
              errorMessage =
                errorMessage || "Tidak bisa dihapus karena ada paket terkait";
              break;
            case 500:
    ...
    async deleteProgramConfirm2() {
      ...
      try {
        ...
        if (response.data.code === 200) {
          ...
          this.showNotification(
            response.data.message || "Gagal menghapus paket",
            false
          );
        }
      } catch (error) {
    ...
    updateHargaFields() {
      ...
      const tripleField = document
        .getElementById("triple")
        .closest(".form-row");
      const doubleField = document
    ...
    async addPaket() {
      ...
      if (quad === null && triple === null && double === null) {
        this.showNotification(
          "Minimal salah satu harga (quad/triple/double) harus diisi!",
          false
        );
        return;
      }
      ...
      if (response.data.code === 200) {
        ...
        this.showNotification(
          "Gagal menambahkan paket: " + response.data.message,
          false
        );
      }
    },
    ...
    handleErrorResponse(error) {
      if (error.response) {
        switch (error.response.status) {
          case 400:
            this.showNotification(
              "Format data tidak valid: " +
                (error.response.data.message || "Periksa input Anda"),
              false
            );
            break;
          case 403:
            this.showNotification(
              "Program ini sudah memiliki maksimal 4 paket!",
              false
            );
            break;
          case 404:
          ...
          case 409:
            this.showNotification(
              "Nama paket sudah ada dalam program ini!",
              false
            );
            break;
          default:
  ...
  <div style="display: flex; align-items: center; gap: 10px;">
    ...
    formatRupiah(input) {
      ...
      if (original !== "" && /[^\d]/.test(original)) {
        ...
        if (!onlySeparators) {
          this.showNotification(
            "Karakter selain angka tidak diperbolehkan!",
            false
          );
        }
      }
  ...
  mounted() {
    ...
    hotelTextarea.addEventListener("focus", () => {
      if (starSelect.value === "") {
        this.showNotification(
          "Harap pilih Star (Bintang) terlebih dahulu!",
          false
        );
        starSelect.focus();
    ...
    quadInput.addEventListener("focus", () => {
      if (itemHargaSelect.value === "") {
        this.showNotification(
          "Harap pilih Jumlah Item Harga terlebih dahulu!",
          false
        );
  ...
  beforeUnmount() {
    ...
    tripleInput.removeEventListener("input", () =>
      this.formatRupiah(tripleInput)
    );
    doubleInput.removeEventListener("input", () =>
      this.formatRupiah(doubleInput)
    );
...
.search-icon {
  ...
  width: 18px;
  height: 18px;
  background: url('data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="%23888"><path d="M10 2a8 8 0 1 1-5.293 14.707l-3.388 3.39a1 1 0 0 1-1.414-1.414l3.388-3.39A8 8 0 0 1 10 2Zm0 2a6 6 0 1 0 4.243 10.243A6 6 0 0 0 10 4Z"></path></svg>')
    no-repeat center;
...
.clear-icon {
  ...
  width: 18px;
  height: 18px;
  background: url('data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="%23FF0000"><path d="M18 6L6 18M6 6l12 12" stroke="%23FF0000" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>')
    no-repeat center;
  background-size: cover;
  cursor: pointer;
}
